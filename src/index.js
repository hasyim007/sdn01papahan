  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/data' && request.method === 'GET') {
      ...
      return new Response(data, {
        headers: {
          ...
          // PENTING: jangan pernah di-cache — halaman publik & admin harus SELALU
          // menerima data terbaru dari D1, tidak boleh ada versi basi yang nyangkut
          // di cache browser/edge (ini salah satu penyebab "perubahan admin tidak
          // muncul di publik" kalau tidak diberi header ini).
    ...
    if (url.pathname === '/api/data' && request.method === 'PUT') {
      ...
      try {
        ...
        // PENTING: Cloudflare D1 punya batas KERAS 2.000.000 byte (~2MB) untuk
        // satu baris/kolom. Kita kasih sedikit ruang aman di bawah itu (1.8MB)
        // supaya penolakan terjadi DI SINI dengan pesan yang jelas, bukan
        // gagal diam-diam / crash di D1. Kalau admin menaruh foto sebagai link
        // (mis. Google Drive) alih-alih unggah file, ukuran data akan jauh di
        // bawah batas ini karena yang tersimpan cuma teks URL-nya saja.
        const MAX_BYTES = 1.8 * 1024 * 1024;
        if (bodyText.length > MAX_BYTES) {
          return json({
            error: 'Data situs terlalu besar untuk disimpan (' +
              (bodyText.length / (1024 * 1024)).toFixed(2) + 'MB, maksimal ~1.8MB). ' +
              'Ini biasanya karena banyak foto diunggah langsung dari perangkat. ' +
              'Gunakan opsi "tempel link Google Drive/URL gambar" untuk foto-foto berukuran besar ' +
              'alih-alih mengunggah file, supaya ukurannya jadi kecil.'
          }, 413, env, request);
        }
      ...
      try {
        ...
        // Tangkap error D1 (mis. row too large) supaya admin dapat pesan yang
        // jelas dan bisa dilihat di UI, bukan sekadar "Failed to fetch".
        console.error('D1 write error:', e);
        return json({
          error: 'Gagal menyimpan ke database server: ' + (e && e.message ? e.message : String(e)) +
            '. Coba kurangi ukuran/jumlah foto, atau gunakan link Google Drive untuk foto.'
        }, 500, env, request);
      }
