...
const SETTINGS_KEYS = [
  ...
  'kontak', 'footer', 'pageOrder', 'notification',
];
...
async function callGeminiGenerateBerita(apiKey, { tanggal, penulis, kategori, judul, poin }) {
  const systemPrompt = [
    'Kamu adalah staf humas SD Negeri (sekolah dasar negeri di Indonesia) yang menulis berita singkat untuk website sekolah.',
    'Tugasmu: dari data & poin-poin yang diberikan admin, tulis draft berita dalam Bahasa Indonesia baku namun hangat, gaya jurnalistik sekolah -- BUKAN gaya artikel AI generik.',
    'ATURAN GAYA (WAJIB dipatuhi):',
    '- Jangan gunakan frasa klise/khas tulisan AI seperti "dalam rangka", "tak hanya ... tetapi juga", "hal ini menunjukkan bahwa", "tidak dapat dipungkiri", "di era yang serba", "sebagai bentuk", "diharapkan dapat", dipakai berulang-ulang.',
    '- Variasikan struktur kalimat, jangan template/kaku, jangan bertele-tele.',
    '- Tulis apa adanya berdasarkan poin yang diberikan, JANGAN mengarang fakta, nama, angka, atau kutipan yang tidak ada di poin.',
    '- Panjang isi lengkap cukup 2-4 paragraf pendek, wajar untuk berita sekolah dasar.',
    'Balas HANYA dengan JSON valid (tanpa markdown, tanpa teks lain di luar JSON), persis format ini:',
    '{"excerpt": "1-2 kalimat ringkasan untuk kartu berita", "content": "isi lengkap, paragraf dipisah dengan \\n\\n", "tags": "tag1, tag2, tag3"}',
  ].join('\n');
...
async function buildDecomposeStatements(env, data) {
  ...
  return { stmts, changedBeritaSlugs };
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/api/admin/berita/generate' && request.method === 'POST') {
      ...
      try {
        const draft = await callGeminiGenerateBerita(settings.geminiApiKey, { tanggal, penulis, kategori, judul, poin });
